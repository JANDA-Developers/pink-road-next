...
export const setNextPop = (
    id: string,
    date: Date = dayjs().add(1, "m").toDate()
) => {
...
export const usePopups = (
    defaultModals: Ipopup[],
    config: Partial<IUsePopupsConfig> = defaultConfig
) => {
    ...
    const openAllAuto = () => {
        for (const popup of popups) {
            const validDate =
                dayjs(popup.startDate).isBefore(new Date()) &&
                dayjs(popup.endDate).isAfter(new Date());
            const nextPopDateStr =
                localStorage.getItem("popup" + popup._id) || "";
            const nextPopDate = nextPopDateStr
                ? new Date(nextPopDateStr)
                : new Date();
            const lastOpenCheck =
                !nextPopDate || dayjs(nextPopDate).isAfter(new Date());
    ...
    useEffect(() => {
        if (config.autoOpen) openAllAuto();
    }, []);
