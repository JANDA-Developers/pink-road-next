...
import {
    BANK_DEPOSIT_CONFIRM,
    BANK_REFUND,
    PAYMENT_LIST,
    SETTLEMENT_CAL,
} from "../apollo/gql/payment";
import {
    bankDepositConfirm,
    bankDepositConfirmVariables,
    bankRefund,
    bankRefundVariables,
    Fpage,
    Fpayment,
    paymentList,
    paymentListVariables,
    settlementCal,
    settlementCalVariables,
    _PaymentFilter,
    _PaymentSort,
} from "../types/api";
...
interface IuseItemListProp
    extends Partial<ListInitOptions<_PaymentFilter, _PaymentSort>> {
    options?: QueryHookOptions<paymentList, paymentListVariables>;
}
export interface IusePaymentList
    extends IListHook<_PaymentFilter, _PaymentSort> {
    items: Fpayment[];
    getLoading: boolean;
    pageInfo: Fpage;
}
...
export const useSettlementCal = ({
    ...
    const { data } = useQuery<settlementCal, settlementCalVariables>(
        SETTLEMENT_CAL,
        {
            variables: {
                filter,
...
export const useBankDepositConfirm = generateMutationHook<
    bankDepositConfirm,
    bankDepositConfirmVariables
>(BANK_DEPOSIT_CONFIRM, {
    ...getRefetch(BOOKING_FIND_BY_CODE, BOOKING_LIST, COUNT),
});
export const useBankRefund = generateMutationHook<
    bankRefund,
    bankRefundVariables
>(BANK_REFUND, {
    ...getRefetch(BOOKING_FIND_BY_CODE, BOOKING_LIST, COUNT),
