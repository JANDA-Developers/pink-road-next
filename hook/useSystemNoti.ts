...
export const useSystemNotiRead = generateMutationHook<
    systemNotiRead,
    systemNotiReadVariables
>(SYSTEMNOTI_READ, { ...getRefetch(SYSTEMNOTI_LIST, UNREAD_SYSTEMNOTIS) });
export const useSystemNotiHide = generateMutationHook<
    systemNotiHide,
    systemNotiHideVariables
>(SYSTEMNOTI_HIDE, { ...getRefetch(SYSTEMNOTI_LIST, UNREAD_SYSTEMNOTIS) });
export const useUnReadSystemNotiFind = () => {
    const skip = !Storage?.getLocal("jwt", "");
    const { data } = useQuery<unReadSystemNotiFind>(UNREAD_SYSTEMNOTIS, {
        pollInterval: 180000,
        skip,
    });
