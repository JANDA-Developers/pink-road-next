...
export const Question: React.FC<Ipage> = (pageInfo) => {
  ...
  const gotoView = (inq: questionList_QuestionList_data) => () => {
    ...
    const isMyProductQuestion =
      myProfile?._id === inq.product?.author?._id && myProfile !== undefined;
    ...
    if (!inq.isOpen) {
      if (!isMyProductQuestion && !isManager && !isMyQuestion) {
        passwordModalHook.openModal({
          callBack: (password) => {
            router.push(
              "/member/question/view/" + inq._id + "/?pw=" + password
            );
          },
          title: "패스워드를 입력 해주세요",
          messageLabel: "패스워드",
          ...
          <div className="alignment">
            <div className="left_div">
              <ul className="board_option__btn">
                ...
                <li
                  onClick={handleSetFilter(QuestionStatus.READY)}
                  className={checkOnStatus(QuestionStatus.READY)}
                >
                  <a>
                    미답변<strong>{/*{unAnsweredQuestionCount}*/}</strong>
                  </a>
                </li>
              </ul>
            </div>
