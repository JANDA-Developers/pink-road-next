...
import { BookingStatus, ProductReOpenReq, productElseReq, ProductStatus, _ProductFilter } from '../../../types/api';
...
export const MsGoodsMain: React.FC<IProp> = () => {
    const [fitlerType, setFilterType] = useState<TProductFilterKey>("title_contains");
    const { items, filter, setFilter, setSort, sort, viewCount, setViewCount, setUniqFilter, setPage, pageInfo, getLoading } = useProductList();
    ...
    const uniqStatusFilterList: (keyof _ProductFilter)[] = ["status_eq", "determined_eq", "elseReq_eq"];
    const doSearch = (search: string) => {
        setUniqFilter(fitlerType, ["title_contains", "code_eq", "exField__sellerName_eq", "exField__sellerNickName_eq", "keyWards_in", "regionLabel_eq"], search);
    }
                    ...
                    <div className="con_box_top pb5">
                        ...
                        <MasterSearchBar onDateChange={hanldeCreateDateChange} Option={
                            <select onChange={(e) => {
                                ...
                                <option value={"title_contains" as TProductFilterKey}>상품명</option>
                                <option value={"code_eq" as TProductFilterKey}>상품번호</option>
                                <option value={"exField__sellerName_eq" as TProductFilterKey}>판매자명</option>
                                <option value={"exField__sellerNickName_eq" as TProductFilterKey}>판매자닉네임</option>
                                <option value={"keyWards_in" as TProductFilterKey}>키워드</option>
                                <option value={"regionLabel_eq" as TProductFilterKey}>지역</option>
                            </select>
                            ...
                            LeftDiv={
                                <>
                                    ...
                                    <ul style={{ marginBottom: ".5rem" }} className="board_option">
                                        ...
                                        <li onClick={setType(ProductStatus.COMPLETED)} className={checkOnStatus(ProductStatus.COMPLETED)}><a>판매완료<strong>{compeltedProductCountMaster}</strong></a></li>
                                    </ul>
                                    <ul className="board_option">
                                        ...
                                        <li onClick={setType(ProductStatus.UPDATE_REQ_REFUSED)} className={checkOnStatus(ProductStatus.UPDATE_REQ_REFUSED)}><a>수정반려<strong>{updateRequestRefuseCountMaster}</strong></a></li>
                                        ...
                                        <li onClick={setType(ProductStatus.READY)} className={checkOnStatus(ProductStatus.READY)}><a>생성요청<strong>{createRequestCountMaster}</strong></a></li>
                                    </ul>
                                </>
                            }
                            Sort={
                        ...
                        <div className="con_box_body master__table">
                            ...
                            {items.map((item, i) =>
                                <div key={item._id} className="list_line">
                                    ...
                                    <div className="td04"><div className="goods__info_title"><Link href={tourSearchLink({ title: item.title })}><a className="title"> {item.title} <ReqBadge req={item.elseReq} /></a></Link></div></div>
