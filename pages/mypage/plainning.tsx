              ...
              Status={
                <div className="jul2">
                  ...
                  <div className="text">
                    ...
                    <span
                      onClick={handleSatus(ProductStatus.CANCELD)}
                      className={checkStatusOn(ProductStatus.CANCELD)}
                    >
                      취소
                    </span>
                    <span
                      onClick={handleSatus(ProductStatus.OPEN)}
                      className={checkStatusOn(ProductStatus.OPEN)}
                    >
                      판매중
                    </span>
                    <span
                      onClick={handleSatus(ProductStatus.COMPLETED)}
                      className={checkStatusOn(ProductStatus.COMPLETED)}
                    >
                      완료
                    </span>
                    <span
                      onClick={handleSatus(ProductStatus.UPDATE_REQ)}
                      className={checkStatusOn(ProductStatus.UPDATE_REQ)}
                    >
                      수정요청
                    </span>
                    <span
                      onClick={handleSatus(ProductStatus.REFUSED)}
                      className={checkStatusOn(ProductStatus.REFUSED)}
                    >
                      생성반려
                    </span>
                    <span
                      onClick={handleSatus(ProductStatus.UPDATE_REQ_REFUSED)}
                    ...
                    <ul>
                      {items.map((item, i) => (
                        <li key={item._id}>
                          ...
                          <div className="th02">
                            <span className="m_title">유형: </span>
                            {itemTypeToKr(item.type)}
                          </div>
                          <div className="th03">
                            <span className="m_title">개시일: </span>
                          ...
                          <div className="th04">
                            ...
                            <div className="info goods__info_title">
                              <span className="ct">{item.category?.label}</span>
                              <span className="g-number">
                                상품번호: {item.code}
                              </span>
                              <strong className="title">
                                {item.title} <ReqBadge req={item.elseReq} />
                              </strong>
                            </div>
                          </div>
                          <div className="th05">
                            <span className="m_title">출발일: </span>
                          ...
                          <div className="th06">
                            ...
                            <span className="people">
                              성인 {item.bookerSummary.adultCount}
                            </span>
                            <span className="m_title"> / </span>
                            <span className="people">
                              소아 {item.bookerSummary.kidsCount}
                            </span>
                            <span className="m_title"> / </span>
                            <span className="people">
                              유아 {item.bookerSummary.babyCount}
                            </span>
                          </div>
                          <div className="th07">
                            {/* 단위 : 건 */}
                          ...
                          <div className="th08">
                            ...
                            <span className="present">
                              예약 {item.bookerSummary.completeBookCount}
                            </span>
                            <span className="m_title"> / </span>
                            <span className="present">
                              대기 {item.bookerSummary.readyBookCount}
                            </span>
                            <span className="m_title"> / </span>
                            <span className="present">
                              환불 {item.bookerSummary.readyBookCount}
                            </span>
                          </div>
                          <div className="th09">
                            <i className="btn">
                              <Link href={`/tour/write/${item._id}`}>
                                <a>상품수정</a>
                              </Link>
                            </i>
                            {/*글수정으로 가기 */}
                            ...
                            {/* POPUP */}
                            {item.status === ProductStatus.OPEN && (
                              <i
                                onClick={handleHandWriteModal(item._id)}
                                className="btn"
                              >
                                예약등록
                              </i>
