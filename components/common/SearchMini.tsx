...
export const SearchMini: React.FC<IProp> = ({ onSubmit, className }) => {
    ...
    return <div className={`search_mini ${className}`}>
        <div className="in">
            <input onKeyPress={whenEnter(() => { onSubmit(value) })} value={value} onChange={(e) => { setValue(e.currentTarget.value) }} type="text" placeholder="검색 내용을 입력해주세요." />
            <svg onClick={() => {
                onSubmit(value)
            }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30.94 31.44">
