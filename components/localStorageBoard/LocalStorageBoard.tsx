        ...
        <div className="LocalStorageBoard__blocks">
            {items?.map(item =>
                <div className="LocalStorageBoard__block" key={item._id}>
                    <h3 className="LocalStorageBoard__title">{item.simpleData.title} <span className="LocalStorageBoard__date">{dayjs(item.pickupAt).format("YYYY.MM.DD일 hh시 mm분")}</span></h3>
                    <div className="LocalStorageBoard__btns">
                        <button className="LocalStorageBoard__btn btn small" onClick={handleLoad(item)}>불러오기</button>
