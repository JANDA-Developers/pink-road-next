        ...
        <div className="in_txt statement_popup master_popup">
          ...
          <a className="close_icon" onClick={closeModal("#SettlementModal")}>
            ...
            <div className="statement_popup__topBox info_txt">
              ...
              <span className="r-number">
                정산상태: <i>{settlementStatus(settlement.status)}</i>
              </span>
            ...
            <div className="alignment">
              <div className="left_div">
                <h4>
                  <i>
                    {yyyymmdd(createdAt)} ~ {yyyymmdd(startDate)} 예약
                  </i>
                ...
                <ul>
                  {bookingsPagination.slice.map((bk) => (
                    <li className="settlementModal__li" key={bk._id}>
                      ...
                      <div className="th03">
                        {paymentStatus(bk.payment?.status)}
                      </div>
            ...
            <div className="alignment ">
              <div className="left_div">
                <h4>
                  <i>취소건이</i>이 총 <strong>{canceldBooking.length}</strong>
                  건
                </h4>
              </div>
              <div className="right_div" />
                ...
                <ul>
                  {cancelBookingsPagination.slice.map((cb) => (
                    <li className="settlementModal__li" key={cb._id + "cancel"}>
                      ...
                      <div className="th06">
                        {autoComma(cb.bookingPrice)}원 |{" "}
                        {autoComma(cb.payment?.totalCancelPrice)} [
                        {cb.payment?.isPartialCancel}]
                      </div>
                      <div className="th07">
                        <strong className={bookingStatusColor(cb.status)}>
                          {bookingStatus(cb.status)}
                        </strong>
                      </div>
                    </li>
            ...
            <div className="sum_div mt50">
              ...
              <ul className="last_ul">
                <li>
                  <div>
                    <i>=</i>총 정산 예상금 :{" "}
                    <strong>{autoComma(settlement.settlementPrice)}</strong>원
                  </div>
                </li>
              </ul>
            </div>
