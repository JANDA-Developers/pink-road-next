...
export const ProductControllers: React.FC<IProp> = ({
    product,
    acceptCreate,
    acceptUpdate,
    ...
    const { isManager, isParterB, isParterNonB, role, isSeller } =
    ...
    const determindAble =
        (left7overDay || isManager || isParterB) &&
        product?.determined === false &&
        product.status === ProductStatus.OPEN;
    // 출발 안하기: 일반파트너일경우 출발 7일 전일때
    const unDetermindAble =
        (left7overDay || isManager || isParterB) &&
    ...
    const cancelAvailable =
        (noPeople || isManager || isParterB) &&
        status === ProductStatus.OPEN &&
        product?.determined === false; // Travel 캔슬함수 사용하면됨
    // 삭제 가능: 사람없을때 || 오픈이거나 마감 상태가 아닐떄
    const deleteAvailable =
        (noPeople || isManager) &&
        product &&
        DELETE_AVAIABLE_PRODUCTS.includes(status);
    // 다시 오픈이 가능한가
    const reopenAvailable =
        (isManager || isParterB) && status === ProductStatus.CANCELD;
    // 다시 오픈 요청이 가능한가
    const reopenReqAB =
        !isManager &&
        isParterNonB &&
        status === ProductStatus.CANCELD &&
        product?.elseReq !== ProductReOpenReq.REOPEN;
    // 다시 오픈 요청 접수가 가능한가
    const elseReopenAcceptAB =
        isManager && product?.elseReq === ProductReOpenReq.REOPEN;
    // 다시 오픈 요청 거절이 가능한가
    const elseReopenDenyAB =
        isManager && product?.elseReq === ProductReOpenReq.REOPEN;
    ...
    const elseReqHandle = (elseReq: ProductReOpenReq) => {
        if (elseReq === ProductReOpenReq.REOPEN)
            return handleReqElse(ProductReOpenReq.REOPEN);
    };
    ...
    const rejectHandle = (() => {
        ...
        if (proptTarget?.target === "travelWithdrwal")
            return handleTravelWidthWral;
    ...
    return (
        <div className="side__form">
            ...
            {determindAble && (
                ...
                >
                    ...
                    <i
                        ...
                        data-tip={CONDITION.travelDetermineChange}
                    />
                </button>
            )}
            ...
            {cancelAvailable && (
                ...
                >
                    ...
                    {isSeller && (
                        <i
                            className="jandaicon-info2 tooltip"
                            data-for="TooltipProductModal"
                            data-tip={CONDITION.travelCacnel}
                        />
                    )}
                </button>
            )}
            ...
            {reopenReqAB && (
                <button
                    onClick={elseReqHandle(ProductReOpenReq.REOPEN)}
                    type="submit"
                    className="btn medium"
                >
                    재개요청
            ...
            {elseReopenAcceptAB && (
                ...
                >
                    ...
                    <i
                        className="jandaicon-info2 tooltip"
                        data-for="TooltipProductModal"
                        data-tip={`상품요청: ${reqToKr(product?.elseReq)}`}
                    />
                </button>
            )}
            ...
            {/* 생성허용 */}
            {isManager && status === ProductStatus.READY && (
                <button
            ...
            {/* 업데이트 허용 */}
            {isManager && status === ProductStatus.UPDATE_REQ && (
                <button
