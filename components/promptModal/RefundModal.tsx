...
export interface IRefundModalSubmit
    extends Omit<BankRefundInput, "bookingId"> {}
...
export const RefundModal: React.FC<IProp> = ({ onSubmit, booking }) => {
    ...
    return ReactDOM.createPortal(
        <Modal
            inClassName="refundModal"
            id="RefundModal"
            title={"예약 환불 및 취소하기"}
        >
            <div className="refundModal__refundAmt"></div>
            <div className="refundModal__inputBox">
                <span>
                    <span className="refundModal__inputBoxLabel">환불금액</span>
                    <span className="refundModal__refundAmtLabel">
                        (환불가능
                    </span>
                    :{autoComma(cancelAvailableAmt)}원)
                </span>
                <div className="refundModal__inputWrap">
                    <input
                        className="refundModal__input"
                        value={autoComma(submitData.cancelPrice)}
                        onChange={(e) => {
                            submitData.cancelPrice =
                                toNumber(e.currentTarget.value) || 0;
            ...
            <div className="input_box">
                <textarea
                    className="refundModal__textarea"
                    onChange={(e) => {
            ...
            <div className="info">
                <p>
                    <i className="flaticon-flag-1" /> 한번 여행취소시 다시
                    되돌릴 수 없습니다. 취소는 신중하게 부탁드립니다.
                </p>
            </div>
            <div className="fin">
