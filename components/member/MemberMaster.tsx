...
export const MemberMaster: React.FC<IProp> = ({
  ...
  const handleStopUser = (userIds?: string[]) => {
    if (!confirm(`정말로 유저 ${selectedIds.length}명을 정지 시키겠습니까?`))
      return;
    stopUsers({
      variables: {
        reason: "",
      ...
      <div className="in partner">
        ...
        <div className="in_content">
          ...
          <div className="con family">
            ...
            {isBusiTap && <h5 className="tap__title">개인파트너 회원</h5>}
              ...
              <MasterSearchBar
                ...
                Option={
                  ...
                  >
                    ...
                    <option value={"phoneNumber_eq" as TuniqSearch}>
                      휴대폰
                    </option>
                  </select>
                }
              ...
              <MasterAlignMent
                Sort={
                  <SingleSortSelect {...singleSort}>
                    ...
                    <option value={_UserSort.createdAt_desc}>
                      가입일 &darr;
                    </option>
                ...
                LeftDiv={
                  <div>
                    <ul className="board_option">
                      <li
                        onClick={setIsForeginer(undefined)}
                        className={checkOnAllForgien()}
                      >
                        <a>
                          전체<strong>{totalIndiMemberCount}</strong>
                        </a>
                      </li>
                      <li
                        onClick={setIsForeginer(false)}
                        className={checkOnForeginer(false)}
                      >
                        <a>
                          내국인<strong>{koreanMemberCount}</strong>
                        </a>
                      </li>
                      <li
                        onClick={setIsForeginer(true)}
                        className={checkOnForeginer(true)}
                      >
                        <a>
                          외국인<strong>{foreginMemberCount}</strong>
                        </a>
                      </li>
                    </ul>
                    <ul className="board_option">
                      ...
                      <li
                        onClick={setGenderFilter(GENDER.MAIL)}
                        className={checkOnGender(GENDER.MAIL)}
                      >
                        <a>
                          남<strong>{koreanMemberCount}</strong>
                        </a>
                      </li>
                      <li
                        onClick={setGenderFilter(GENDER.FEMALE)}
                        className={checkOnGender(GENDER.FEMALE)}
                      >
                        <a>
                          녀<strong>{foreginMemberCount}</strong>
                        </a>
                      </li>
                    </ul>
                  </div>
