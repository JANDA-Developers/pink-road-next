    ...
    <div className="basket_box">
      <div className="basket_list">
        ...
        {items.map((item, i) => (
          <div key={item._id} className="td">
            ...
            <div className="t02">
              ...
              ></div>
              <div className="right">
                <div className="ct">{item?.category?.label}</div>
            ...
            <div className="t03">
              <div className="day">
                출발일 :{" "}
                <strong>{dayjs(item.startDate).format("MM.DD (W)")}</strong>
              </div>
              <div className="start_where">출발장소 : {item.startPoint}</div>
              <div className="tour_mode">
                여행방식 :{getTypeTextOfProduct(item.type, item.dateRange)}
              </div>
              <div className="men">
                선택인원 : <strong>총 {getTotalCount(item.count)}명</strong>
                {` - 성인${item.count.adult}, 소인${item.count.kids}, 유아${item.count.baby}`}
              </div>
              <button onClick={handleModify(item)} className="btn option_btn">
                조건 추가/변경
              </button>
            </div>
            <div className="t04">
            ...
            <div className="t05">
              ...
              <div className="men_cunt">
                모집 인원 : <strong>{item.compeltePeopleCnt}</strong> /{" "}
                {item.maxMember}
              </div>
        ...
        <div className="baket_bottom">
          ...
          <div className="sum02">
            <strong>{autoComma(totalPrice)}원</strong>
          </div>
        </div>
