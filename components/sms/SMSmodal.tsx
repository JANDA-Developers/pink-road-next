...
import {
    useSmsTemplateCreate,
    useSmsTemplateDelete,
    useSmsTemplateUpdate,
...
export const SMSmodal: React.FC<IProps> = ({ template }) => {
    ...
    const defaultTag =
        (template?.tags[0]?.value as TSMStemplateTag) || "MEMBER";
    // get templates

    const [smsTag, setTag] = useState<TSMStemplateTag>(defaultTag);
    const [update] = useSmsTemplateUpdate({
        onCompleted: ({ SmsTemplateUpdate }) => {
            if (SmsTemplateUpdate.ok) closeModal("#SMSmodal")();
        },
    });
    const [create] = useSmsTemplateCreate({
        onCompleted: ({ SmsTemplateCreate }) => {
            if (SmsTemplateCreate.ok) closeModal("#SMSmodal")();
        },
    });

    const [deleteMu] = useSmsTemplateDelete({
        onCompleted: ({ SmsTemplateDelete }) => {
            if (SmsTemplateDelete.ok) closeModal("#SMSmodal")();
        },
    });

    const [trigger, setTrigger] = useState<
        NotificationTriggerCreateInput | ITriggerDefault
    >(
        cloneObject(template?.trigger) || {
            event: null,
            isEnabled: false,
        }
    );
                ...
                <div className="sms-box">
                    ...
                    <div className="form right">
                        ...
                        <p className="infotxt_gray">
                            80Byte 이하일땐 80Byte 이상시 LMS입니다. 템플릿
                            문자에 따라서 정확하지 않을 수 있습니다.
                        </p>
                    </div>
                </div>
